...
/*   Updated: 2023/03/07 11:09:11 by wangthea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	animate_static_player(t_game *g)
{
	...
	if (frame % 1000 == 0)
	{
		if (g->map.map[i][j] == space || g->map.map[i][j] == player)
		{
			...
			{
				display_image(g, g->txtr.idle[frame / 1000].sprite,
					i, j);
			}
			else if (g->directions.left == true)
			{
				display_image(g, g->txtr.idle_left[frame / 1000].sprite,
					i, j);
			}
			else if (g->directions.right == true)
			{
				display_image(g, g->txtr.idle_right[frame / 1000].sprite,
					i, j);
			}
		}
	}
...
static int	animate_static_slimes(t_game *g)
{
	...
	while (index < g->map.slimes.slimes)
	{
		...
		if (frame % 1000 == 0 && g->end == false && g->directions.hurt == false)
			display_image(g, g->txtr.sl_mv_down[frame / 1000].sprite, i, j);
		else if (frame % 1000 == 0 && g->end == false && g->directions.hurt == true)
		{
			if (g->directions.down == true)
			{
				display_image(g, g->txtr.link_down_hurt[frame / 1000].sprite,
					g->map.player.pos_y, g->map.player.pos_x);
			}
			else if (g->directions.left == true)
			{
				display_image(g, g->txtr.link_left_hurt[frame / 1000].sprite,
					g->map.player.pos_y, g->map.player.pos_x);
			}
			else if (g->directions.right == true)
			{
				display_image(g, g->txtr.link_right_hurt[frame / 1000].sprite,
					g->map.player.pos_y, g->map.player.pos_x);
			}
			else
			display_image(g, g->txtr.link_up_hurt[frame / 1000].sprite, g->map.player.pos_y, g->map.player.pos_x);
		}
		index++;
	}
	frame++;
